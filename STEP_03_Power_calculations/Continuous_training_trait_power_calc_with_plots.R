...
study <- "Cognitive_ability"
nsnp <- 200000 # Number of SNPs in summary statistics file
cases <- 329821 # # Number of cases used to generate summ stats (check ancestry in paper)
ctrls <- 0 
binary <- c( TRUE, FALSE ) # Training trait, then target trait
prevalence <- 0.01 # Ignored
...
polygenescore( nsnp=nsnp, n=n, vg1=vg1[1], cov12=cov12[1], pi0=pi0[1], pupper=c(0,1),
               nested=nested, weighted=weighted, binary=binary,
               prevalence=prevalence, sampling=sampling, lambdaS=lambdaS,
               shrinkage=shrinkage, logrisk=logrisk, alpha=alpha[1], r2gx=r2gx,
               corgx=corgx, r2xy=r2xy, adjustedEffects=adjustedEffects, riskthresh=riskthresh )$power
...
ggplot( pi0_df, aes( x = Thresholds, y = FDR, col = Value))+
   geom_point()+
  geom_line()+
